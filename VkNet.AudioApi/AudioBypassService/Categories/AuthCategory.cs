...
namespace VkNet.AudioApi.AudioBypassService.Categories;

public partial class AuthCategory : Abstractions.Categories.IAuthCategory
{
    ...
    private static Regex AnonTokenRegex() => 
        new Regex("\"anonymous_token\"\\:\\s?\"(?<token>[\\w\\.\\=\\-]*)\"\\,?", RegexOptions.Multiline);

    private static Regex AuthVerifyHashRegex() =>
        new Regex("\"code_auth_verification_hash\"\\:\\s?\"(?<hash>[\\w\\.\\=\\-]*)\"\\,?", RegexOptions.Multiline);
